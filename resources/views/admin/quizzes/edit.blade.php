...
@section('topbar-title', 'Edit Quiz')
...
<div class="mb-8 rounded-2xl relative overflow-hidden animate-fade-up"
    ...
    <div class="relative p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 z-10 w-full flex-wrap">
        <div>
            <h1 class="text-3xl font-black mb-2 tracking-tight text-white">Edit Quiz</h1>
        ...
        <div class="xl:col-span-2 flex flex-col gap-6">
            ...
            <div class="card">
                <div class="card-inner">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" class="form-textarea" rows="2">{{ old('description', $quiz->description) }}</textarea>
                    </div>

                    <div class="mb-4">
                        <label class="form-label">Quiz Type *</label>
                        <div style="display:flex;gap:1rem;flex-wrap:wrap;">
                            <label style="display:flex;align-items:center;gap:0.5rem;cursor:pointer;padding:0.75rem 1rem;border:1px solid rgba(255,255,255,0.1);border-radius:10px;flex:1;min-width:180px;">
                                <input type="radio" name="quiz_type" value="lesson_quiz" id="type-lesson"
                                       {{ old('quiz_type',$quiz->quiz_type)==='lesson_quiz'?'checked':'' }}
                                ...
                                <div>
                                    ...
                                    <div style="color:rgba(255,255,255,0.4);font-size:0.75rem;">Attached to a specific lesson.</div>
                                </div>
                            </label>
                            <label style="display:flex;align-items:center;gap:0.5rem;cursor:pointer;padding:0.75rem 1rem;border:1px solid rgba(255,255,255,0.1);border-radius:10px;flex:1;min-width:180px;">
                                <input type="radio" name="quiz_type" value="between_lessons" id="type-between"
                                       {{ old('quiz_type',$quiz->quiz_type)==='between_lessons'?'checked':'' }}
                                       style="accent-color:#930056;" onchange="toggleLessonPicker()" />
                                <div>
                                    <div style="color:#fff;font-size:0.85rem;font-weight:600;">📋 Standalone Quiz</div>
                                    <div style="color:rgba(255,255,255,0.4);font-size:0.75rem;">Placed between lessons.</div>
                                </div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div id="order-picker" class="mb-4">
                        ...
                        <input type="number" name="order" value="{{ old('order', $quiz->order) }}" step="0.5" min="0" class="form-input" />
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-inner">
                    ...
                    <div id="no-questions-msg" style="text-align:center;padding:2rem;color:rgba(255,255,255,0.3);font-size:0.9rem;display:none;">
            ...
            <div class="card">
                <div class="card-inner">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="passing_score" value="{{ old('passing_score',$quiz->passing_score) }}" min="0" max="100" class="form-input" />
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Time Limit (minutes, 0 = none)</label>
                        <input type="number" name="time_limit_minutes" value="{{ old('time_limit_minutes',$quiz->time_limit_minutes) }}" min="0" class="form-input" />
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Max Attempts</label>
                        <input type="number" name="max_attempts" value="{{ old('max_attempts',$quiz->max_attempts) }}" min="1" class="form-input" />
                    </div>
                    <div class="flex items-center gap-2 mb-2">
                        <input type="checkbox" name="randomize_questions" id="rand_q" value="1"
                               {{ $quiz->randomize_questions?'checked':'' }} style="width:16px;height:16px;accent-color:#930056;" />
                        <label for="rand_q" class="text-sm" style="color:rgba(255,255,255,0.6);cursor:pointer;">Randomize questions</label>
                    </div>
                    <div class="flex items-center gap-2">
                        <input type="checkbox" name="show_result_immediately" id="show_r" value="1"
                               {{ $quiz->show_result_immediately?'checked':'' }} style="width:16px;height:16px;accent-color:#930056;" />
...
function addQuestion(data = {}) {
    ...
    const typeOptions = types.map(t => `<option value="${t.val}" ${data.type===t.val?'selected':''}>${t.label}</option>`).join('');
    ...
    div.innerHTML = `
        ...
        <div class="mb-3">
            ...
            <textarea name="questions[${i}][question]" class="form-textarea" rows="2" required style="font-size:0.875rem;">${data.question||''}</textarea>
        </div>
        <div style="display:flex;gap:1rem;margin-bottom:1rem;flex-wrap:wrap;">
            <div style="flex:1;min-width:180px;">
                <label class="form-label" style="font-size:0.8rem;">Type</label>
                <select name="questions[${i}][type]" class="form-select" style="font-size:0.875rem;" onchange="renderAnswerSection(${i}, this.value)">
                    ${typeOptions}
                </select>
            </div>
            <div style="width:100px;">
                <label class="form-label" style="font-size:0.8rem;">Points</label>
                <input type="number" name="questions[${i}][points]" value="${data.points||1}" min="1" class="form-input" style="font-size:0.875rem;" />
            </div>
        </div>
        <div id="answers-${i}" class="mb-3"></div>
        <div>
            <label class="form-label" style="font-size:0.8rem;">Explanation</label>
            <input type="text" name="questions[${i}][explanation]" value="${data.explanation||''}" class="form-input" style="font-size:0.875rem;" placeholder="Why is this the correct answer?" />
        </div>`;
    document.getElementById('questions-container').appendChild(div);
    renderAnswerSection(i, data.type || 'multiple_choice', data);
}

function removeQuestion(i) {
    const el = document.getElementById(`q-${i}`);
    if (el) el.remove();
    if (!document.querySelector('#questions-container [id^="q-"]'))
        document.getElementById('no-questions-msg').style.display = 'block';
}

function renderAnswerSection(i, type, data={}) {
    const c = document.getElementById(`answers-${i}`); if (!c) return; c.innerHTML = '';
    if (type === 'multiple_choice') {
        const opts = data.options || [{option_text:'',is_correct:false},{option_text:'',is_correct:false}];
        c.innerHTML = `<label class="form-label mb-2" style="font-size:0.8rem;">Answer Options</label>
            <div id="opts-${i}">${opts.map((o,j)=>renderOption(i,j,o)).join('')}</div>
            <button type="button" onclick="addOption(${i})" style="margin-top:0.5rem;color:#a78bfa;background:none;border:1px dashed rgba(167,139,250,0.4);border-radius:6px;padding:0.3rem 0.8rem;font-size:0.78rem;cursor:pointer;">+ Add Option</button>`;
    } else if (type === 'true_false') {
        const correct = data.correct_answer || 'true';
        c.innerHTML = `<label class="form-label mb-2" style="font-size:0.8rem;">Correct Answer</label>
        ...
        c.innerHTML = `<label class="form-label mb-1" style="font-size:0.8rem;">Correct Answer / Keywords</label>
            <input type="text" name="questions[${i}][correct_answer]" value="${data.correct_answer||''}" class="form-input" style="font-size:0.875rem;"
                   placeholder="${type==='fill_in_the_blank'?'Expected answer':'Expected keywords'}" />`;
    }
}
...
function renderOption(qi, oi, data={}) {
    return `<div id="opt-${qi}-${oi}" style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.4rem;">
        <input type="checkbox" name="questions[${qi}][options][${oi}][is_correct]" value="1" ${data.is_correct?'checked':''}
               style="width:16px;height:16px;accent-color:#930056;flex-shrink:0;" title="Correct" />
